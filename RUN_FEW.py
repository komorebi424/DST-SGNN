...
parser.add_argument('--data', type=str, default='ETTh2_test', help='data set')
parser.add_argument('--data_pretrain', type=str, default='ETTh2_pretrain', help='data set')
...
parser.add_argument('--test_train', type=float, default=0.2)
...
result_train_file = os.path.join('output_best', args.data_pretrain, 'train')
...
train_set = Data(root_path=data_info_pretrain['root_path'], flag='train', seq_len=args.seq_length, pre_len=args.pre_length,
                 type=data_info_pretrain['type'], train_ratio=args.petrain_train, val_ratio=args.petrain_val)

finetuning_set = Data(root_path=data_info['root_path'], flag='train', seq_len=args.seq_length, pre_len=args.pre_length,
...
def test():
    result_test_file = 'output_best/' + args.data_pretrain + '/train'
...
if __name__ == '__main__':
    if args.is_training:
        ...
        for epoch in range(args.train_epochs):
            ...
            for index, (x, y) in enumerate(finetuning_dataloader):
                cnt += 1
                y = y.float().to("cuda:0")
                x = x.float().to("cuda:0")
