...
class SGSC(nn.Module):
    def __init__(self, pre_length, embed_size, seq_length,
        ...
        self.A = (self.patch_num * self.feature_size)
        self.B = self.d_model

        self.w1 = nn.Parameter(self.scale1 * torch.randn(2, self.A, self.B))
        self.b1 = nn.Parameter(self.scale1 * torch.randn(2, self.B))
        self.w2 = nn.Parameter(self.scale1 * torch.randn(2, self.A, self.B))
        self.b2 = nn.Parameter(self.scale1 * torch.randn(2, self.B))


        self.fc12 = nn.Linear(self.patch_num, 12).double()
        ...
        self.c_in = feature_size * self.patch_num
    ...
    def GC(self, x):
        ...
        w10 = torch.einsum('bpi,ij->bpj', prr.transpose(2, 1), self.w1[0])
        w20 = torch.einsum('bpi,ij->bpj', prr.transpose(2, 1), self.w2[0])
    ...
    def forward(self, x):
        ...
        x = x.permute(0, 2, 1).contiguous().detach()
        B, N, L = x.shape
        ...
        x = x1.reshape(B, self.feature_size, self.patch_num, self.embed_size)
        x = x.permute(0, 1, 3, 2)
        x = x.double()

        x = self.fc12(x)

        x = x.reshape(B, self.feature_size, -1)
