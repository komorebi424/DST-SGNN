...
def MAPE(v, v_, axis=None):
    epsilon = 1e-8
    return np.mean(np.abs((v_ - v) / (v + epsilon)), axis).astype(np.float64)

def sMAPE(v, v_, axis=None):
    epsilon = 1e-8
    return np.mean(2 * np.abs(v_ - v) / (np.abs(v) + np.abs(v_) + epsilon), axis).astype(np.float64)
...
def evaluate(y, y_hat, by_step=False, by_node=False):
    ...
    if not by_step and not by_node:
        return MAE(y, y_hat, axis=None), MSE(y, y_hat, axis=None), sMAPE(y, y_hat, axis=None)
